...
const sketch = ({context, width, height}) => {
	...
	const bgTexture = new THREE.TextureLoader().load('/media/images/desert.png');
	...
	const material = new THREE.MeshPhysicalMaterial({color: 0xdc7c71, roughness: 0.1, transmission: 1, thickness: 0.5, envMap: hdrEquirect, envMapIntensity: 0.5, clearcoat: 1, clearcoatRoughness: 0.01, normalScale: new THREE.Vector2(0.5), normalMap: normalMapTexture, clearcoatNormalMap: normalMapTexture, clearcoatNormalScale: new THREE.Vector2(0.5)});
	...
	const instanceData = [...Array(MESH_COUNT)].map(() => {
		const position = new THREE.Vector3(5.0 * (-1 + 2 * Math.random()), 5.0 * (-1 + 2 * Math.random()), 0.5 + (-1 + 2 * Math.random()));
